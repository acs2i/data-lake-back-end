...
router.get(FAMILY + "/search/:value", async( req: Request, res: Response) => {
    try {
        const page: string | any | string[] | undefined = req.query.page;
        const limit: string | any | string[] | undefined = req.query.limit;
        ...
        if(isNaN(Number(value))) {
            ...
            documents  = await FamilyModel.find({ YX_LIBELLE: { $regex: value } }).limit(intLimit);

        } else {
            // need to discuss to convert yx_code into string
            documents = await FamilyModel.find({ YX_CODE: { $regex: value } }).limit(intLimit);
