    ...
    try {
      ...
      if (exportToCsv === "true") {
        try {
          ...
          let allTransformedProducts = [];

          // Utiliser le cursor pour récupérer les données par lots
          const cursor = ProductModel.find(filter)
            .populate({
              path: "brand_ids",
              select: "code name",
            })
          ...
          for await (const product of cursor) {
            allTransformedProducts.push({
              ...
              supplier:
                (product.suppliers?.[0]?.supplier_id as any)?.code || "",
          ...
          if (allTransformedProducts.length > 0) {
            ...
            return res.status(OK).json({
              csvFilePath,
              totalExported: allTransformedProducts.length,
              msg: "Products exported successfully",
            });
          } else {
            return res.status(OK).json({
              msg: "No products found to export",
              totalExported: 0,
            });
          }
        } catch (error) {
