    ...
    try {
        ...
        for (const u of uvc) {
            ...
            if (!u.ean) {
                ...
                if (foundEan) {
                    throw new Error(req.originalUrl + "msg: Ean already exists: " + JSON.stringify(u));
                }
            }
            ...
            if (!newUvc) {
                ...
                throw new Error(req.originalUrl + " msg: uvc save did not work for some reason: " + JSON.stringify(u));
            }
        ...
        const newProduct: Product | null = await new ProductModel({ ...product, uvc_ids, version: 1 });
        ...
        if (!newProduct) {
            ...
            throw new Error(req.originalUrl + " msg: product save did not work for some reason: " + JSON.stringify(product));
        }

        const savedProduct: Product | null = await newProduct.save({ timestamps: true });
